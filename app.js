/* ═══════════════════════════════════════════════════
   ZAHRAT ELMADINA RESTAURANT & CAFE — Customer App v2
...
const i18n = {
  en: {
    ...
    footer:'Authentic flavors, warm moments.',
...
// ── Landing tutorial videos / animated templates ──
...
function landingTemplateHTML(template) {
  const d=landingTemplateData(template);
  return `<div class="landing-animation ${d.theme}"><div class="la-title">${d.title}</div><div class="la-steps">${d.steps.map((x,i)=>`<div class="la-step" style="--i:${i}"><span>${x[0]}</span><b>${x[1]}</b></div>`).join('')}</div><div class="la-caption">${d.caption}</div></div>`;
}
function renderLandingTutorials() {
  const s=App.settings||{}, section=document.getElementById('landing-tutorials');
  if(!section || s.landing_videos_enabled==='no') { if(section) section.classList.add('hidden'); return; }
  section.classList.remove('hidden');
  [['order','order-tutorial-stage'],['book','book-tutorial-stage']].forEach(([kind,id])=>{
    const stage=document.getElementById(id); if(!stage)return;
    const mode=s[`landing_${kind}_video_mode`]||'template';
    if(mode==='disabled'){stage.innerHTML='<div style="color:var(--text3);font-size:13px">Tutorial unavailable.</div>';return;}
    if(mode==='upload' && s[`landing_${kind}_video_url`]) { stage.innerHTML=`<video src="${s[`landing_${kind}_video_url`]}" muted playsinline loop preload="metadata" controls></video>`; return; }
    stage.innerHTML=landingTemplateHTML(s[`landing_${kind}_video_template`]||(kind==='order'?'order_steps':'book_steps'));
  });
}
function openLandingTutorial(kind) {
  const s=App.settings||{}, mode=s[`landing_${kind}_video_mode`]||'template';
  const root=document.getElementById('modal-root'); if(!root)return;
  const title=kind==='order'?'How to Order':'How to Book a Table';
  let body='';
  if(mode==='upload' && s[`landing_${kind}_video_url`]) body=`<video src="${s[`landing_${kind}_video_url`]}" controls autoplay playsinline style="width:100%;max-height:65vh;border-radius:14px;background:#000"></video>`;
  else body=landingTemplateHTML(s[`landing_${kind}_video_template`]||(kind==='order'?'order_steps':'book_steps'));
  root.innerHTML=`<div class="modal-overlay" onclick="if(event.target===this)closeLandingTutorial()"><div class="modal" style="max-width:820px;padding:18px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px"><h2 style="margin:0">${title}</h2><button class="cart-close" onclick="closeLandingTutorial()">✕</button></div>${body}</div></div>`;
}
function closeLandingTutorial(){const root=document.getElementById('modal-root');if(root)root.innerHTML='';}
...
async function init() {
  ...
  try {
    ...
    App.categories = categories.filter(c => c.name !== 'All' && c.enabled !== false);
    App.menu = menu.filter(i => i.enabled !== false);
    App.filteredMenu = App.menu;
    App.pricingRules = rules;
    App.activeDiscount = DB.getActiveDiscount(rules);

    applySettings();
    renderLandingTutorials();
    renderCategories();
    renderFeatured();
    renderMenuGrid();
    renderGoogleReviews();
...
function applySettings() {
  ...
  // ── WiFi ──
  if (el('wifiName')) el('wifiName').textContent = s.wifi_name || 'Zahrat_Guest';
  if (el('wifiPass')) el('wifiPass').textContent = s.wifi_pass || 'Ask staff for Wi-Fi password';
  if (el('footerAddress')) el('footerAddress').textContent = s.contact_address || 'Avenue Mall, Obour City, Egypt';
  if (el('footerHours')) el('footerHours').textContent = `${s.open_time || '10:00 AM'} – ${s.close_time || '12:00 AM'}`;
  ...
  const rName = s.restaurant_name || 'Zahrat Elmadina Restaurant & Cafe';
  const rNameAr = s.restaurant_name_ar || 'مطعم وكافيه زهرة المدينة';
  const displayName = isAr() ? rNameAr : rName;
  // Split on last space: e.g. "Zahrat Elmadina Restaurant & Cafe" → part1="Flavor " part2="House"
  // If no space, show all in part2 (accent)
  let part1 = displayName, part2 = '';
  if (!isAr() && displayName.includes(' Restaurant & Cafe')) {
    part1 = displayName.replace(' Restaurant & Cafe','');
    part2 = 'Restaurant & Cafe';
  } else if (!isAr()) {
    const spaceIdx = displayName.lastIndexOf(' ');
    part1 = spaceIdx > 0 ? displayName.slice(0, spaceIdx + 1) : '';
    part2 = spaceIdx > 0 ? displayName.slice(spaceIdx + 1) : displayName;
  }
...
function injectAIAdvisor() {
  ...
  wrap.innerHTML = `
    <div style="background:linear-gradient(135deg,rgba(31,107,79,.12),rgba(31,107,79,.04));border:1px solid rgba(31,107,79,.25);border-radius:20px;padding:20px 24px">
      ...
      <div style="display:flex;gap:8px">
        <input id="aiMoodInput" placeholder="${t('aiMoodPlaceholder')}"
          style="flex:1;padding:11px 16px;border-radius:50px;background:rgba(255,255,255,.06);border:1px solid rgba(31,107,79,.25);color:var(--text);font-size:14px;outline:none"
...
//  LOYALTY & REWARDS
// ═══════════════════════════════════════════════════
const LOYALTY_REWARD_ICONS_JS = { percent:'🏷️', fixed:'💰', free_addon:'🧀', free_delivery:'🚚' };
...
async function loadCustomerLoyalty() {
  ...
  try {
    ...
    let html = `
      <div style="text-align:center;padding:20px 0;background:linear-gradient(135deg,rgba(31,107,79,.12),rgba(31,107,79,.03));border-radius:16px;margin-bottom:16px">
...
function selectLoyaltyReward(id) {
  ...
  document.getElementById('modal-root').innerHTML = '';
  toast(isAr()?'تم اختيار المكافأة ⭐':'Reward selected ⭐', 'success');
  renderCart(); updateCartBadge();
}
function removeLoyaltyReward() {
  App.selectedLoyaltyReward = null;
  App.cart = App.cart.filter(i => !i._loyaltyRewardLine);
  renderCart(); updateCartBadge();
}
...
function applyFilters() {
  ...
  if (App.activeCategory) {
    const cat = App.categories.find(c => c.id === App.activeCategory);
    if (cat?.enabled === false) App.activeCategory = null;
  }
  if (App.searchQuery) items = items.filter(i => {
    const cat = i.categories || {};
    const haystack = [i.name, i.name_ar, i.description, i.description_ar, cat.name, cat.name_ar, i.tags].filter(Boolean).join(' ').toLowerCase();
    return haystack.includes(App.searchQuery);
  });
  // Dietary filter
...
function changeCartQty(itemId, delta) {
  const idx = App.cart.findIndex(c => c.id === itemId && !c._loyaltyRewardLine);
  if (idx === -1) return;
...
function computeLoyaltyRewardValue(reward, sub) {
  ...
  return 0; // free add-on/free delivery are non-cash perks; redemption is recorded separately
}
function cartTotal() {
  const sub = cartSubtotal();
  const taxRate = parseFloat(App.settings.tax_rate ?? 14) / 100;
  const serviceRate = parseFloat(App.settings.service_charge ?? 10) / 100;
  const tax = sub * taxRate;
  const svc = sub * serviceRate;
  ...
  itemsEl.innerHTML = App.cart.map(item => `
    <div class="cart-item">
      ...
      <div class="cart-item-info">
        ...
        <div class="cart-item-price">${item._loyaltyRewardLine ? `<span style="color:var(--accent);font-weight:800">FREE ⭐</span>` : fmt(item.price)}</div>
        <div class="cart-item-controls">
          ${item._loyaltyRewardLine ? '<span style="font-size:11px;color:var(--accent);font-weight:700">Loyalty reward</span>' : `<button class="cart-qty-btn" onclick="changeCartQty('${item.id}',-1)">−</button>
          <span class="cart-qty-num">${item.qty}</span>
          <button class="cart-qty-btn" onclick="changeCartQty('${item.id}',1)">+</button>`}
        </div>
      </div>
      <span class="cart-item-del" onclick="${item._loyaltyRewardLine ? 'removeLoyaltyReward()' : `removeFromCart('${item.id}')`}">✕</span>
    </div>`).join('') +
    // AI upsell
    (App._upsellItem ? `
    <div style="padding:12px 14px;border-radius:14px;background:rgba(31,107,79,.06);border:1px solid rgba(31,107,79,.2);margin-top:10px">
...
function renderCartLoyaltySection() {
  ...
  if (App.selectedLoyaltyReward) {
    ...
    el.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:rgba(31,107,79,.08);border:1px solid rgba(31,107,79,.25);border-radius:12px;margin-bottom:12px">
    ...
    <div class="modal" style="max-height:none;overflow:visible;margin:auto">
      ...
      <div class="form-group">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;margin-top:4px">
          <label id="pm-cash" onclick="selectPayment('cash')" style="display:flex;align-items:center;gap:12px;padding:14px 16px;border-radius:var(--radius);border:2px solid var(--accent);cursor:pointer;background:rgba(31,107,79,0.08)">
            <span style="font-size:22px">💵</span>
      ...
      <div id="instapay-section" style="display:none;margin-bottom:16px;padding:16px;background:rgba(31,107,79,0.06);border:1px solid rgba(31,107,79,0.2);border-radius:var(--radius)">
...
function selectPayment(method) {
  ...
  ['cash','card','instapay'].forEach(m => {
    ...
    if (el) { el.style.border = m===method ? '2px solid var(--accent)' : '2px solid var(--border2)'; el.style.background = m===method ? 'rgba(31,107,79,.08)' : 'transparent'; }
...
async function loadLoyaltyPreview(phone) {
  ...
  try {
    ...
    if (loyalty) {
      ...
      el.innerHTML = `<div style="background:rgba(31,107,79,.08);border:1px solid rgba(31,107,79,.2);border-radius:12px;padding:10px 14px">
...
async function openShareCard(orderId, orderNum) {
  document.getElementById('modal-root').innerHTML = '';
  const restName = App.settings.restaurant_name || 'Zahrat Elmadina Restaurant & Cafe';
...
function drawShareCanvas(restName, orderNum, couponCode) {
  ...
  // Divider
  ctx.strokeStyle = 'rgba(31,107,79,.3)';
  ctx.lineWidth = 1;
  ...
  ctx.fillText('Authentic flavors, warm moments.', W/2, 380);
}

async function doShare(restName, couponCode) {
  const canvas = document.getElementById('shareCanvas');
  const text = `Just ordered at ${restName}! 🍽️ Amazing food${couponCode ? ` — use code ${couponCode} for 10% off your first visit!` : ''} #ZahratElmadina #FoodLovers`;
  if (navigator.share && canvas) {
    try {
      const blob = await new Promise(r => canvas.toBlob(r, 'image/png'));
      const file = new File([blob], 'zahrat-elmadina.png', { type: 'image/png' });
  ...
  if (canvas) {
    ...
    a.download = 'zahrat-elmadina-meal.png';
    a.click();
...
function renderTrackingCard(order) {
  ...
  if (order.status === 'preparing' && order.prep_started_at) {
    ...
    timerHtml = `
    <div style="background:rgba(31,107,79,.06);border:1px solid rgba(31,107,79,.2);border-radius:14px;padding:16px;margin:20px 0;text-align:left">
...
function openGoogleMaps() {
  ...
  if (link) {
    ...
    const name = encodeURIComponent(App.settings.restaurant_name || 'Zahrat Elmadina Restaurant & Cafe');
